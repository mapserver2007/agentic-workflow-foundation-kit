...
description: >-
  ...
  `create-session-workflow` 等）の `manifest.yaml` + `templates/` から出力ファイルを
...
- **複数の設定スキルから共有される**。`agentic-workflow-foundation`（親）と `create-session-workflow`（子）の両方に対し `--skill-dir` を変えて `generate.py` / `audit.py` を実行する。さらに `agentic-workflow-foundation/scripts/check_design_drift.py` は `genlib` をスキル境界を越えて import する。エンジンを1つの設定スキルに埋め込むと、他スキルが内部に手を伸ばす結合（reach-in）になる。
...
| ファイル | 役割 |
| --- | --- |
| `scripts/genlib.py` | 共有ライブラリ。最小 YAML ローダ（`load_manifest`）/ `sha256_file` / `skill_dir_of` / `root_from_skill_dir` / `YamlError` / テンプレート描画ヘルパ。`generate.py` と `audit.py` が同一描画ロジックを共有するための土台。 |
...
```bash
# 生成（親 → 子の順で設定スキルごとに実行）
python3 .cursor/skills/manifest-generator/scripts/generate.py --skill-dir <config-skill-dir>

# 冪等性ドライラン
python3 .cursor/skills/manifest-generator/scripts/generate.py --skill-dir <config-skill-dir> --check

# 監査（冪等性 + 必須要件）
python3 .cursor/skills/manifest-generator/scripts/audit.py --skill-dir <config-skill-dir>
```
