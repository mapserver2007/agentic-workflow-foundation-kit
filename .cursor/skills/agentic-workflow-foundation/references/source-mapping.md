...
| `deep_thinking` | `.cursor/skills/deep-thinking/SKILL.md`（内部 A/B 並列分析 + C 統合裁定。ユーザー明示起動は自然言語、ワークフロー起動は呼出元 Investigation Memo へ出力を分岐。一次証跡優先の採用順位、同一モデル時の限定性開示、A/B 未回収時の中止規約、静的保証と実行時制約の境界表を含む）/ `config.yaml`（モデル・実行パラメータの SoT。seed モード。`require_distinct_models` / `max_issues_per_round` / `stop_when` を含む）/ `references/analyst-brief.md`（Ledger スキーマ・`不明` 正規値・ブリーフ欠落報告・検査次元カバレッジ契約）/ `../requirement-analysis/references/deep-brief.md`（requirement_analysis 有効時のワークフロー用ブリーフ。空でない検査次元インベントリを渡す契約）/ `references/verification-flags.md`（`MODEL_HOMOGENEOUS` を含む。静的検査との関係を明記）/ `references/issue-card-format.md`（再審三条件・ラウンド配布上限）/ `references/response-synthesis.md`（ユーザー明示起動時のみの一次証跡優先 tie-break）/ `validate_deep_thinking.py`（静的契約ゲート G-DEEP-*。audit 統合）/ `docs/QUALITY_GATE.md §4.1`（静的契約ゲートの検査 ID・対象外の実行時特性） |
...
| `marker_id` | `.gitignore` / `.cursorignore` の固定除外パターン。Meta 常時行（`.cursor/.session/` 等）と認証情報・秘密鍵の除外はテンプレ固定。 |
| `project.ignore_dirs` | `.gitignore` managed ブロック。`tech_contract.provisioning.command_actions[].writes` と postcondition `marker` から `contract_projection.project_ignore_dirs` が決定論的に投影する。技術名推論は行わない。 |
...
2. `project.*` と `github_access` は Phase 1.5 の `init.yaml` → `apply_kit_init.py` で確定する（workflow_pattern は開発型固定、GitHub provider は `github_app | keychain`）。`framework.accd_axes` は開発型の軽量実装として seed manifest に固定する。確定値は root `manifest.yaml` に保存する。
